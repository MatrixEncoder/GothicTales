...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  const filePath = path.join(process.cwd(), `src/content/killers/${params.slug}.md`);
  try {
    const fileContents = fs.readFileSync(filePath, 'utf8');
    const { data } = matter(fileContents);

    return {
      title: data.title || 'Untitled Profile',
      description: data.description || 'A dark profile from the archives',
    };
  } catch (error) {
    return {
      title: 'Profile Not Found',
      description: 'A dark profile from the archives',
    };
  }
}

export default function KillerPage({ params }: { params: { slug: string } }) {
  const { slug } = params;
  const filePath = path.join(process.cwd(), `src/content/killers/${slug}.md`);
