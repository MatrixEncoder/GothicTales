...
export async function generateMetadata({ params }: { params: { id: string } }): Promise<Metadata> {
  const killer = killerProfiles[params.id];
  
  return {
    title: killer ? `${killer.name} | Gothic Tales` : 'Killer Not Found',
    description: killer ? killer.content.substring(0, 160) : 'Killer details not available'
  };
}
